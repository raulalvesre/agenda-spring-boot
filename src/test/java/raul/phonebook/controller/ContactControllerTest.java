...
public class ContactControllerTest {
    ...
    private final String controllerPath = "/contacts";
    ...
    @Test
    @Sql(scripts = "classpath:sql/addContactsToUser.sql")
    void should_retrieve_contact_by_id() throws Exception {
        mockMvc.perform(get(controllerPath + "/3")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.id", is(3)))
                .andExpect(jsonPath("$.name", is("contact3")))
                .andExpect(jsonPath("$.telephones[*].id", containsInAnyOrder(5, 6)))
                .andExpect(jsonPath("$.telephones[*].type", containsInAnyOrder(1, 1)))
                .andExpect(jsonPath("$.telephones[*].description", containsInAnyOrder("tel5", "tel6")))
                .andExpect(jsonPath("$.telephones[*].telephoneNumber", containsInAnyOrder("(11) 97645-4635", "(11) 97645-4636")))
    ...
    void should_return_page() throws  Exception {
        mockMvc.perform(get(controllerPath)
                ...
                .andExpect(jsonPath("$.content[*].name", containsInAnyOrder("contact3", "contact4")))
                .andExpect(jsonPath("$.content[*].telephones[*].id", containsInAnyOrder(5, 6, 7, 8)))
                .andExpect(jsonPath("$.content[*].telephones[*].type", containsInAnyOrder(1, 1, 1, 1)))
                .andExpect(jsonPath("$.content[*].telephones[*].description", containsInAnyOrder("tel5", "tel6", "tel7", "tel8")))
                .andExpect(jsonPath("$.content[*].telephones[*].telephoneNumber", containsInAnyOrder("(11) 97645-4635", "(11) 97645-4636", "(11) 97645-4637", "(11) 97645-4638")))
    ...
    @Test
    @Sql(scripts =  { "classpath:sql/addContactsToUser.sql" , "classpath:sql/addContactsToAdm.sql"})
    void should_return_page_with_only_requester_contacts() throws  Exception {
        mockMvc.perform(get(controllerPath)
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void should_add_new_contact() throws Exception {
        var newContact = ContactDTOFactory.buildValidCreateOrUpdateContactDTO();
    ...
    void should_update_contact() throws Exception {
        var contact = ContactDTOFactory.buildValidCreateOrUpdateContactDTO();

        mockMvc.perform(put(controllerPath + "/3")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void should_delete_contact() throws Exception {
        mockMvc.perform(delete(controllerPath + "/3"))
                .andExpect(status().isNoContent());
    }
    ...
    @Test
    @Sql(scripts = "classpath:sql/addContactsToUser.sql")
    void unprocessable_with_contact_with_telephone_already_registered_on_post() throws Exception {
        var newContact = ContactDTOFactory.buildValidCreateOrUpdateContactDTO();
        newContact.getTelephones().get(0).setTelephoneNumber("(11) 97645-4635");
    ...
    void unprocessable_with_contact_with_telephone_already_registered_on_put() throws Exception {
        ...
        mockMvc.perform(put(controllerPath + "/4")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(jsonMapper.writeValueAsString(newContact)))
                .andExpect(status().isUnprocessableEntity());
    }
    ...
    void should_validate_bad_contact_request_with_duplicate_telephone_numbers() throws Exception {
        var newContact = ContactDTOFactory.buildValidCreateOrUpdateContactDTO();
        newContact.getTelephones().get(1).setType(newContact.getTelephones().get(0).getType());
